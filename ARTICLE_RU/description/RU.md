...
Запустим ядро на настольном машине:
...
Проверим работу энкодеров. Установим робота на какую-нибудь подставку. Руками будем поворачивать колеса робота и при этом наблюдать за углами поворота в мониторе топика.
...
Ту же операцию продлаем и с левым колесом.
...
Используя плагин Message Publisher в `rqt` установим значение ШИМ на моторах в 1023. Моторы начнут вращаться.
...
Запишем максимальные угловые скорости вращения колес нашего робота:

<iframe width="800"
        src="https://www.youtube.com/embed/G-atk6DhUhg"
        title="Abot. Max anglular wheel speed test."
        frameborder="0"
        class="article__cover-youtube"
        allowfullscreen="">
</iframe>

Наше правое колесо вращается с максимальной скоростью  `16.4` рад/с а левое с `15.1` рад/с.
...
Этот тип контроллера, непосредственно отвечает за движение робота. Как это работает? Мы даем контроллеру входные данные. Например, мы хотим чтобы робот ехал с линейной скоростью в 1 метр/c и угловой скростью в 2 рад/c. Контроллер берет эти входные данные, так же он берет значения текущих скоростей с и положение робота полученный с одометрии, обрабатывает все это и на выходе отдает необходимые угловые скорости вращения колес чтобы наш робот двигался именно таким образом.
...
Я установил скорость контроллера на " 100 " раз в секунду.
