...
Если роботу нужно передвигаться только в двухмерном пространстве, всё становится уже проще. Мы можем рассмотреть движение робота как движение материальной точки в плоскости (X, Y) в [прямоугольной или Декартовой системе координат](https://ru.wikipedia.org/wiki/Прямоугольная_система_координат) (X, Y, Z).
...
Например, обычный автомобиль не может взять и подвинуться строго вправо или влево с места — значит, его движение неголономно. С другой стороны, если бы у автомобиля вместо обычных колёс стояли [всенаправленные](https://ru.wikipedia.org/wiki/Всенаправленное_колесо), то он смог бы двигаться голономно.
...
[Двухколёсный дифференциальный привод](https://en.wikipedia.org/wiki/Differential_wheeled_robot) — самый простой и распространённый тип привода в любительской робототехнике. Именно он используется в домашних роботах-пылесосах.
...
К слову, именно такой тип привода имеют наши [Робоняша](prod://robonyasha-iskra-js) и [Драгстер](prod://dragster).
...
Этот тип привода использует особые [Omni-колёса](https://en.wikipedia.org/wiki/Omni_wheel) вместо обычных. Поликолесо — это всенаправленное колесо с небольшими роликами, расположенными по окружности. Оси роликов перпендикулярны оси вращения колеса. Контролируя скорость и направление вращения поликолёс, вы можете заставить робота двигаться в любом направлении — другими словами, сделать его движение голономным.

Обычно шасси с Omni-колёсами насчитывает ровно 3 или 4 колеса. Шасси с тремя колёсами обеспечивает большую тягу, поскольку любая реактивная сила распределяется только через три точки, и робот хорошо сбалансирован даже на неровной местности. Всенаправленные колёса имеют высокую стоимость, поэтому трёхколёсное шасси обходится заметно дешевле, варианты с четырьмя.
...
- Работа поликолёс изначально строится на принципах проскальзывания. Невозможно поставить датчик вращения на каждый ролик Omni-колеса, поэтому полученная одометрия неточная.
...
Этот тип привода использует вместо обычных [колёса Илона (Mecanum)](https://ru.wikipedia.org/wiki/%D0%9A%D0%BE%D0%BB%D0%B5%D1%81%D0%BE_%D0%98%D0%BB%D0%BE%D0%BD%D0%B0), предназначенные для грузоподъёмных и проходимых роботов. По сути это разновидность Omni-колеса, только на шведском колесе Илона ролики по всей окружности обода расположены под углом 45° к плоскости и 45° к оси вращения колеса.
...
Можно отключить видимость 3D моделей и проверьть все дерево элементов вашего описания а так же направление осей систем координат.
...
Для крепления электроники мы спроектировали и напечатали на 3D принтере новую деталь для робота в виде панели или диска. На этой детали мы сделали отверстия для крепежа Raspberry и прочих электронных компонентов. Деталь напечатали на [Prusa i3 MK3S](prod://3d-printer-prusa-i3-mk3s) из [серого PLA-пластика ESUN](prod://3d-printer-filament-esun-pla-plus-grey).
...
С добавлением новой напечатанной детали наш робот внешне немного изменился. А это значит что изменилась его 3D модель. И мы должны отредактировать его URDF описание.

***Важно*** Всегда регистрируйте любые изменения реального робота как в САПР документации так и в URDF описании. Это действие не является обязательным но оно прививает привычку контролировать документацию и поможет вам быстрее находить ошибки программах и что еще важнее причины ошибок.
...
Давайте начнем писать драйверы для железа нашего робота.
...
Внутри пакета создаем папку с именем `src`. В папке с именем `src` в ROS конвенционально хранятся исходные файлы программ.
...
Как будет работать эта нода? Класс `Encoder` который мы описали выше считывает сигналы с энкодеров используя прерывания, накапливает их а затем конвертирует в углы поворота колес (в радианах). Углы поворота колес используются для расчета одометрии робота и для определения текущей скорости вращения колес.

Так же мы создадим класс `EncodersPair` который использует углы поворота колес для расчета:

- Пройденного каждым колесом растояния (`left_wheel_angle`, `right_wheel_angle`). В нашем случае, пройденное расстояние мы храним в виде количества оборотов колеса в радианах.
...
Пройденные колесами расстояния помещаются в сообщения типа `std_msgs/Float64` и публикуются в топики `/abot/left_wheel/angle` и `/abot/right_wheel/angle`.

Теукщие скорости вращения колес так же помещаются в сообщения типа `std_msgs/Float64` и публикуются в топики `/abot/left_wheel/current_velocity` и `/abot/right_wheel/current_velocity`.
...
Например, повернув правое колесо примерно на один оборот. Значение угла поворота колеса в топике `/abot/right_wheel/angle` должно соответственно измениться с `0` до примерно `2 * PI`.
...
Для управления скоростью вращения колес мы будем использовать [PID-контроллер](мhttps://ru.wikipedia.org/wiki/ПИД-регулятор). Каждому колесу будет присвоен свой PID-контроллер. Задача PID-контроллера - поддержание необходимой скорости вращения колеса.

Реализацию программы PID-контроллера писать самим не обязательно, в ROS уже есть готовый пакет - [`pid`](http://wiki.ros.org/pid).
...
- На вход PID-контроллера (Input или State) поступает текущая скорость вращения колеса (в радианах в секунду). Текущая скорость вращения считается нодой энкодеров и публикуюется в топики `/abot/right_wheel/current_velocity` и `/abot/left_wheel/current_velocity`.
...
Этим файлом мы запустим две ноды PID-контроллера. Эти ноды подписываются на топики `/abot/right_wheel/target_velocity`, `/abot/left_wheel/target_velocity`, `/abot/right_wheel/current_velocity` и `/abot/left_wheel/current_velocity`.

Оба PID-контроллера работают с частотой в 100 герц (за это отвечают параметры `min_loop_frequency` и `max_loop_frequency`). Для корректировки скорости наши PID-контроллеры публикуют значения ШИМ в топики `/abot/right_wheel/pwm` и `/abot/left_wheel/pwm`. 

***Важно*** Значения ШИМ находятся в диапазоне от [-10.23 , 10.23]. Мы решили отправлять значения ШИМ именно в таком диапазоне, а не в диапазоне от [-1023 , 1023]. Это обусловлено особенностями подбора коэффициентов контроллера - `Kp`, `Ki`, `Kd` в ROS пакете `pid`. По нашему опыту, в ROS не очень часто можно встретить целочисленные значения. Большинство ROS нод обмениваются сообщениями со значениями типа [floating-point (с_плавающей_запятой)](https://ru.wikipedia.org/wiki/Число_с_плавающей_запятой) и в диапазоне от [-1.0 , 1.0]. 

На этом этапе мы задаем стандартные значения коэффициентов `Kp`, `Ki`, `Kd`. Настраивать коэффициенты будем чуть позже.

#### Нода моторов

Теперь мы можем написать окончательную ROS ноду для моторов робота. Назовем ее `dc_motors`.

Создадим файл `dc_motors.cpp` в папке `about_driver/src`.

Как будет работать нода моторов? Нода подписывается на два топика ROS - `/abot/right_wheel/pwm` и `/abot/left_wheel/pwm`. Через эти топики нода получает сообщения типа `std_msgs/Float64`. Эти сообщения содержат значения ШИМ сигналов коррекции скоростей в диапазоне [-10.23 , 10.23]. Далее эти значения умножаются на 100 и отправляются на H-мосты через обьекты класса `DCMotorWiringPi`.
...
Давайте создадим новый файл запуска для всех созданных нод чтобы нам не приходилось каждый раз запускать все драйверы вручную. 

В пакете `abot_driver` в папке `launch` создадим файл запуска `abot_drivers.launch`. В нем запустим созданные нами ноды а так же включим файл запуска PID-контроллеров:
...
Теперь мы можем запускать все наши драйверы одновременно и одной командой. Запустим драйверы на Raspberry:
...
![part_8_rpi_side_screen_6.png](../media/part_8/rpi_side/part_8_rpi_side_screen_6.png)
...
Чтобы публиковать значения в топики вручную через терминалы, используем ROS утилиту [`rqt`](http://wiki.ros.org/rqt).
...
Сперва откроем плагин для публикации сообщений в топик. Нажмем **Plugins → Topics → Message Publisher**. В появившимся окне добавим два наших топика задания скоростей - `/abot/left_wheel/target_velocity` и `/abot/right_wheel/target_velocity`. В эти топики будем публиковать сообщения типа `std_msgs/Float64`. В столбце `rate` установим частоту публикации сообщений в 100 герц. Для запуска процесса публикации нужно установить галочку слева от имени топика.
...
Еще откроем плагин для настройки коэффициентов PID-контроллера в реальном времени. Нажмем **Plugins → Dynamic Reconfigure**.

![part_8_desk_side_screen_5.png](../media/part_8/desk_side/part_8_desk_side_screen_5.png)

На этом шаге вам нужно скоректировать коэффициенты обоих PID-контроллеров. Нужно добиться того чтобы колесо вращалось именно с заданной скоростью. Так же желательно добиться плавной регулировки скорости, без резких разгонов и торможений. Чем больше дейстительная скорость вращения соответствует реальной тем легче будет реальзовать навигацию робота.
...
Сейчас в описании нашего робота левое и правое колесо представлены в виде 3D модели взятой из .STL файла. Колесо круглой формы из .STL файла является лишь набором полигонов. Контроллеру же нужна точная геометрия колеса, то есть колесо должно быть описано математически.

Для этого мы создим еще два сегмента в URDF описании `abot_description`. Назовем их `left_wheel` и `right_wheel`. В этих сегментах мы опишем визуальную составлящую `<visual>` не 3D моделью а геометрией `<geometry>`. Теперь сочленения `left_wheel_to_base` и `right_wheel_to_base` будут вести к этим двум новым сегментам колес. Старые сегменты с 3D моделями мы не будем удалять а оставим их зафиксированными на новых. Новые сегменты колес описываем как цилиндры (`<cylinder>`) высотой 26 мм и радиусом 32,5 мм.
...
Не забываем пересобирать наш проект при добавлении в него новых исходных C++ файлов:
...
![part_9_rpi_side_screen_3.png](../media/part_9/rpi_side/part_9_rpi_side_screen_3.png)
...
Тем не менее колесная одометрия не является точной. Со временем данные одометрии начинают дрифтить (odometry drift) и в ней накаливаются ошибки. Точность одометрии достигается настройской драйверов робота и параметров контроллера движения.
...
Перезагружаем Raspberry. Проверяем что Bluetooth модуль Raspberry вклюяился и работает (`UP RUNNING`):
...
Чтобы джойстик перешел в режим спаривания нажмите одновременно кнопки **Share** и **PS4** и держите их одну - две секунды. Светодиодный индикатор джойстика начнет быстро мигать (быстрее обычного).
...
Теперь, пока светодиод джойстика быстро мигает (идет режим спаривания), создадим новую пару устройств. Так же добавим наш джойстик в список доверенных устройств чтобы при включении джойстика подключение устанавливалось автоматически.
...
Эти датчики могут быть простыми или сложными, работать в 2D или 3D пространстве. Это могут быть цифровые камеры, камеры глубины ([Time-of-Flight](https://ru.wikipedia.org/wiki/Time-of-Flight-камера)), лазерные/ультразвуковые/инфракрасные дальномеры, простые бинарные контактные датчики (бамперы) и так далее. Чем больше датчиков установлено на роботе и чем лучше они синхронизированы друг с другом, тем больше информации робот может получить об окружающем его мире. ROS поддерживает широкий спектр датчиков. Подробнее читайте в [документации на сенсоры в ROS](http://wiki.ros.org/Sensors).
...
Вокруг нашего робота, на уровне лидара появятся красные точки обозначающие окружающие робота преграды. При желании вы можете настроить отображение этих так как нравится вам в настройках `rviz`.

Наш робот начинает видеть! 

<iframe width="1280"
        height="720"
        src="https://www.youtube.com/embed/TuQ89r0-iBg"
...
Попробуйте так же порулить роботом с джойстика и поездить помещению с включенной визуализацией лидара. Взгляните на то как робот видит окружающий мир.

### Теория навигации

Создание автономных мобильных роботов - сложная задача, требующая огромных теоретических и практических знаний. Попробуем рассказать вам о навигации роботов как можно проще, так как мы ее понимаем.

Автономная навигация робота строится на трех фундаментальных принципах:

- Построение карт -[Mapping](https://en.wikipedia.org/wiki/Robotic_mapping)
...
Задача Path planning состоит в том, чтобы ответить на вопрос робота "Как я могу добраться до определенной точки на карте?". Целевая точка на карте может быть установлена оператором робота или же самим роботом. Робот должен уметь cамостоятельно спланировать траекторию движения к целевой точке на карте и добраться до этой точки. Кроме этого траектория движения должна быть безопасна для робота и эффективна.
...
Что из всего этого будем использовать мы? Прежде всего мы составим глобальную карту помещения где наш робот будет работать. Для этого мы будем использовать метод SLAM.

Затем мы дадим роботу эту карту помещения и будем управлять им на ней задавая целевые точки (Goal points). До этих точек робот должен будет добираться самостоятельно обьезжая все препятствия на пути, при этом не потеряв себя на карте. Таким образом мы будем использовать метод Active localisation.
...
В папке `launch` создадим новый файл запуска для метода SLAM. Назовем его так же как и пакет `abot_slam.launch`. Этим файлом мы запустим ноду `slam_gmapping` пакета `gmapping` а так же загрузим SLAM параметры на параметрический сервер ROS.
...
Чем больше площадь помещения вы охватите тем лучше. Ездить роботом желательно плавно, без резких ускорений и торможений. Для более качественной карты нужно проехать роботом вдоль всех стен и вокруг всех предметов на полу. Для большей точности можно проехать по одним и тем же местам несколько раз.

<iframe width="1280"
        height="720"
        src="https://www.youtube.com/embed/G_WgqKymDv8"
        title="Abot. SLAM."
        frameborder="0"
        class="article__cover-youtube"
        allowfullscreen="">
...
В процессе езды допускается незначительное проскальзывание колес и дрифт, такие ошибки решатся встроенной в `gmapping` локализацей. Однако не нужно вручную поднимать робота, переставлять его с места на место или поворачивать - такие ошибки исправить не получится и вам придется запускать SLAM по новой.
...
Готовые карты сохраняются в виде файла расширения .yaml и графического изображения в формате .pgm.
...
Чистим нашу карту в [GIMP](https://ru.wikipedia.org/wiki/GIMP). Так вы глядит наша отредактированная карта:

![part_12_maps_map_2.png](../media/part_12/maps/part_12_maps_map_2.png)

### Стек навигации ROS

Когда карта готова, можно начать реализацию навигации на ней и планирование маршрута.

Для навигации робота в ROS мы используем так называемый [`navigation stack` (стек пакетов навигации)](http://wiki.ros.org/navigation?distro=noetic). Этот стек содержит множество инетересных пакетов и реализованных функций для 2D навигации роботов. Главная особенность `navigation stack` в том что он работает исключительно на роботах построенных с использованием инфраструктуры ROS.

Для работы стека ваш робот должен иметь:

- ROS топик одометрии


Он содержит много интересных пакетов. Суть стека заключается в том, что он берет данные из одометрии, данные с датчиков, положение цели робота на карте и вычисляет скорость робота для контроллера движения.
